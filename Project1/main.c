...
void init() 
{
    createBuffer(); // функци€ создани€ вершин
    initVao();
    ...
    int len = sizeof(shaders) / sizeof(shaders[0]); // определ€ем колво шейдеров
    createProg(shaders, len); // создаЄм программу, содержащую эти шейдеры
    int i = 0;
    for (; i < len; i++) {
...
void createBuffer() // функци€ создани€ вершин
{
    GLushort indices[] = 
    {
        0, 1, 2,
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW); // функци€ заполнени€, которой говорим, что переменна€ на которую указывает эрей бафер, должна быть размером sizeof(vertices), а также, что данные должны быть скопиированы, с помощью GL_STATIC_DRAW гвоорим, что данные измен€ть не будем  
...
void display() {
    glClearColor(0.0f, 1.0f, 1.0f, 0.0f); // цвет дл€ очистки окна
    glClearDepth(1.0f); 
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);// функци€ очистки, использую€ща€ цвет из буфера
    ...
    glutSwapBuffers(); // чтобы содержимаое по€вилось на окне, нужно свапнуть буферы
    glutPostRedisplay();
}
...
int main(int argc, char* argv[]) 
{
    ...
    glutInitWindowSize(600, 600);// создаЄм окно 600 на 600
    glutInitWindowPosition(100, 100);// позици€ 100 100
